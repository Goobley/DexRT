...
struct CoreAndVoigtData {
    ...
    KOKKOS_FORCEINLINE_FUNCTION EmisOpac emis_opac(
...
struct CoreAndVoigtData3d {
    ...
    void init(i64 buffer_len, i32 max_kr);
    /// Fills mip0
    void fill(const State3d& state, i32 la) const;
    void compute_mip_n(const State3d& state, const MipmapComputeState3d& mm_state, i32 level) const;

    KOKKOS_FORCEINLINE_FUNCTION EmisOpac emis_opac(
        const CavEmisOpacState& args
    ) const {
        JasUnpack(args, ks, krl, lambda, vel, phi);
        ...
        // const fp_t a_damp_ratio = square(lambda / lambda0);
        // const fp_t emis_opac_ratio = lambda0 / lambda;
        const fp_t a_damp_ratio = a_damp_ratios(krl);
        const fp_t emis_opac_ratio = emis_opac_ratios(krl);
        const fp_t lambda0 = lambda0s(krl);
        a *= a_damp_ratio;
