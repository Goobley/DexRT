...
inline fp_t simple_conserve_pressure(State* state) {
    ...
    parallel_for(
        ...
        YAKL_LAMBDA (i64 k) {
            ...
            if (new_nhtot < FP(0.0)) {
                printf("Nhtot driven negative @ k = %" PRId64 ", clamped to 1e-5\n", k);
                new_nhtot = FP(1e-5);
            }
    ...
    yakl::fence();
    int max_change_x = max_change_loc % state->atmos.nh_tot.extent(0);
    max_change_loc /= state->atmos.nh_tot.extent(0);
