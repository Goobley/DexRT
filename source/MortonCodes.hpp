...
YAKL_INLINE uint32_t encode_morton<3>(const Coord3& coord) {
    return (part_1_by_1(uint32_t(coord.z)) << 2) + (part_1_by_1(uint32_t(coord.y)) << 1) + part_1_by_1(uint32_t(coord.x));
}
...
YAKL_INLINE Coord3 decode_morton<3>(uint32_t code) {
    return Coord3 {
        .x = i32(compact_1_by_1(code)),
        .y = i32(compact_1_by_1(code >> 1)),
        .z = i32(compact_1_by_1(code >> 2))
    };
}
