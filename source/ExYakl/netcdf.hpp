...
// NOTE(cmo): This is modified from yakl to work directly with kokkos views.
...
namespace ExYakl {
  ...
  public:
    ...
    void write(yakl::Array<T,rank,myMem,myStyle> const &arr , std::string varName , std::vector<std::string> dimNames) {
    ...
    template <class T, int rank, int myMem, int myStyle>
    void read(yakl::Array<T,rank,myMem,myStyle> &arr , std::string varName) {
      // Make sure the variable is there and is the right dimension
      auto var = file.getVar(varName);
      ...
      if ( ! var.isNull() ) {
        ...
        } else if (myStyle == yakl::styleFortran) {
      ...
      if (myMem == yakl::memDevice) {
        ...
        if (std::is_same<T,bool>::value) {
          yakl::Array<int,rank,yakl::memHost,myStyle> tmp("tmp",dimSizes);
          var.getVar(tmp.data());
    ...
    void read(
      ...
      if ( ! var.isNull() ) {
        ...
        bool createArr = ! arr.is_allocated();
        if (arr.is_allocated()) {
          for (int i=0; i < dimSizes.size(); i++) {
            if (dimSizes[i] != arr.extent(i)) {
              #ifdef KOKKOS_ENABLE_DEBUG
