...
void ClassicEmisOpacData::init(i64 buffer_len, i32 wave_batch) {
    dynamic_opac = Fp2d("dynamic_opac", buffer_len, wave_batch);
}

void MultiResMipChain::init(const State& state, i64 buffer_len, i32 wave_batch) {
...
void MultiResMipChain::fill_mip0_atomic(
    const State& state,
...
/// necessary.
void MultiResMipChain::compute_mips(const State& state, int la_start, int la_end) const {
    ...
    KView<i32*> mippable_entries("mippable entries", block_map.num_active_tiles);
    KView<i32*> max_mip_level("max mip entries", block_map.num_z_tiles * block_map.num_x_tiles);
...
void MultiResMipChain::compute_subset_mips(const State& state, const CascadeCalcSubset& subset) const {
    if (state.config.mode == DexrtMode::GivenFs) {
        return;
    }
